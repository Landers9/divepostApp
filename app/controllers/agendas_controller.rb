  ...
  def destroy
    if current_user == @agenda.user || current_user == @agenda.team.owner
      @agenda.team.users.each do |user|
        DeleteAgendaMailer.delete_agenda_mailer(current_user.email, user.email, @agenda.title).deliver
      end
      @agenda.destroy
      redirect_to dashboard_url, notice: "Agenda deleted"
    else
      redirect_to dashboard_url, notice: 'You are not authorized to do this action'
    end
  end
